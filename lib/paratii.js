...
const Paratii = function (opts) {
  ...
  async function deployContracts () {
    ...
    await paratiiRegistry.methods.registerAddress('ParatiiAvatar', paratiiAvatar.address).send()
    await paratiiRegistry.methods.registerAddress('ParatiiToken', paratiiToken.address).send()
    await paratiiRegistry.methods.registerAddress('SendEther', sendEther.address).send()
    await paratiiRegistry.methods.registerAddress('VideoRegistry', videoRegistry.address).send()
    await paratiiRegistry.methods.registerAddress('VideoStore', videoStore.address).send()
    await paratiiRegistry.methods.registerAddress('UserRegistry', userRegistry.address).send()

    await paratiiRegistry.methods.registerUint('VideoRedistributionPoolShare', web3.utils.toWei('0.3')).send()

    paratiiAvatar.methods.addToWhitelist(videoStore.address).send()
  ...
  async function getContractAddress (name) {
    ...
    try {
      let address = await getParatiiRegistry().methods.getContract(name).call({
        from: account.address
      })
      return address
    } catch (err) {
  ...
  async function diagnose () {
    ...
    let registry = await getParatiiRegistry()
