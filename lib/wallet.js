...
import { Accounts } from 'meteor/accounts-base' // TODO - remove
import { showModal } from '/imports/lib/utils.js' // TODO - remove showmodal
import Meteor, { Session } from 'meteor' // TODO - remove this line and Meteor references. for now this line suppresses the npm run lint warning so I can git commit
import { add0x } from './utils'
// import { getUserPTIAddress } from '/imports/api/users.js' // TODO
  ...
  lightwallet.keystore.createVault(opts, function (err, keystore) {
    ...
    keystore.keyFromPassword(password, function (error, pwDerivedKey) {
      ...
      if (key) {
        ...
        Meteor.call('users.update', { 'profile.ptiAddress': add0x(address) }) // TODO
      } else {
...
function mergeOrCreateNewWallet (password) {
  ...
  if (password !== undefined) {
    if (anonymousKeystore !== null) {
      ...
      getSeedFromKeystore('password', anonymousKeystore, function (err, seedPhrase) {
        ...
        createKeystore(password, seedPhrase, key, function (error, result) {
          ...
          showModal('showSeed') // TODO remove
    ...
    showModal('regenerateKeystore') // TODO remove
  }
}
...
function restoreWallet (password, seedPhrase, cb) {
  const userId = Meteor.userId() // TODO
  return createKeystore(password, seedPhrase, userId, cb)
}

function sendTransaction (password, contractName, functionName, args, value, userPTIAddress, callback) {
  ...
  const fromAddr = userPTIAddress // getUserPTIAddress()
