...
contract('Videos: ', function (accounts) {
  ...
  it('should register a video', async function () {
    ...
    let tx = await videoRegistry.create(videoId, videoOwner, price, ipfsHash, ipfsData, {from: accounts[1]})
  ...
  it('anyone can register a video', async function () {
    ...
    return videoRegistry.create(videoId, videoOwner, price, ipfsHash, ipfsData, {from: accounts[1]})
  })

  it('only registrar or the Avatar can unregister a video', async function () {
    let videoRegistry = await Videos.new()
    await videoRegistry.create(videoId, videoOwner, price, ipfsHash, ipfsData, {from: accounts[1]})
  ...
  it('only registrar or the Avatar update the information of a video', async function () {
    ...
    await videoRegistry.create(videoId, videoOwner, 0, 'new_hash_1', 'new_hash_2', {from: accounts[1]})
    videoInfo = await videoRegistry.get(videoId)
