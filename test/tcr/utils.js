...
const utils = {
  getVoting: async () => {
    const tcr = await Tcr.deployed()
    console.log('tcr addrs: ', tcr.address)
    let votingAddr
    try {
      votingAddr = await tcr.voting.call()
      const name = await tcr.name.call()
      console.log('votingAddr :', votingAddr, 'name: ', name)
    } catch (e) {
  ...
  approvePLCR: async (address, adtAmount) => {
    const registry = await Tcr.deployed()
